  ...
  if (!requestData) {
    return (
      <div className="p-10 text-center space-y-4">
        <p className="text-slate-500">Data pengajuan {requestId} tidak ditemukan.</p>
        <button onClick={() => router.push('/fund-requests')} className="text-primary hover:underline">Kembali ke Daftar Pengajuan</button>
      </div>
    );
  }

  const status = STATUS_CONFIG[requestData.status?.toLowerCase()] || STATUS_CONFIG.pending;
  const requesterName = requestData.requester || 'User';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1 flex-wrap">
            <h2 className="text-2xl lg:text-3xl font-bold text-slate-900 tracking-tight">Detail Pengajuan Dana #{requestData.id}</h2>
          ...
          <p className="text-xs text-slate-500">Diajukan pada {requestData.requestDate || requestData.date || '-'}</p>
        </div>
        <div className="flex gap-2 self-start sm:self-auto">
          <button onClick={() => router.push('/fund-requests')} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-xl transition-all text-sm font-semibold shadow-sm cursor-pointer">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8">
              ...
              <div>
                ...
                <p className="text-sm text-slate-900 font-medium">{requestData.neededDate || '-'}</p>
              </div>
              ...
              <div className="col-span-full">
                ...
                <div className="flex items-center gap-3 mt-1">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{requestData.department || 'Unknown Department'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-lg border border-slate-200 bg-slate-50 group hover:border-primary/50 transition-colors">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Uploaded on {requestData.requestDate || '-'}</p>
                  </div>
                </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
            ...
            <div className="relative pl-4 space-y-6 before:absolute before:inset-y-2 before:left-1.75 before:w-px before:bg-slate-200">
              ...
              <div className="relative">
                ...
                <p className="font-mono text-xs text-slate-400 mt-1">{requestData.requestDate || requestData.date || '-'}</p>
              </div>
